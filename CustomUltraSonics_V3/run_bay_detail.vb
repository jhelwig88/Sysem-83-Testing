...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(run_bay_detail))
...
#Region " Overrides "
    Protected Overrides Function ProcessCmdKey(ByRef msg As System.Windows.Forms.Message, ByVal keyData As System.Windows.Forms.Keys) As Boolean
        ...
        Select Case keyCode
            Case Keys.L, Keys.F1, Keys.F2, Keys.F3, _
                 Keys.F4, Keys.F5, Keys.F6, Keys.F7, _
...
#Region " Fill Grid "
    Public Sub Fill_Grid()
        ...
        ' --------------------------
        p_sql = "Select pre_run_id, P1.last_name + ', ' + P1.first_name + ' ' + P1.middle_initial As [Patient], " & _
...
#Region " Get Numbering "
    Private Sub get_process_num(ByVal p_mode As String)
        ...
        ' ==============================
        p_sql = "SELECT IsNull(Max(process_num), 0) " & _
                "FROM run_info_header " & _
                "WHERE error IS NULL AND demo_mode = 0 " & _
                "And end_time Is Not Null " & _
        ...
        If p_mode = "START" Then
            p_sql &= "SELECT Count(*) " & _
                     "FROM run_info_header " & _
                     "WHERE error IS NULL " & _
                     "And start_time Is Not Null And end_time Is Null " & _
                     "And run_header_id <> '" & m_run_header_id & "'; "
        End If
    ...
    Private Function get_procedure_num() As Integer
        ...
        ' =============================
        p_sql = "SELECT IsNull(Max(procedure_num), 0) " & _
                "FROM run_info_data"
        p_dataset = g_framework.Get_Data(p_sql)
...
#Region " Get Process Data "
    Public Function get_process_data_csv_file(ByVal p_id As String, ByRef p_bay_title As String) As String
        ...
        ' * Get Process Details
        ' =====================
        p_sql = "Select a.bay_number, title, detergent, liquid_sterilant, lub_secs, " & _
                "final_disinfect_temp " & _
...
#Region " Load Machine Cycles "
    Private Sub load_machine_cycles(ByVal p_profile_id As String)
        ...
        p_sql = "SELECT cycle_number, cycle_name, default_override " & _
                ...
                "WHERE PC1.profile_id = '" & p_profile_id & _
        ...
        Select Case UCase(type)
            ...
            Case "PURGE"  ' Do System Purge
                ...
                Else
                    ...
                    p_sql = "INSERT INTO purge_history (user_id, bay_number, purge_started, new_disinfect_lot) " & _
            ...
            Case "FINISH"
                ...
                If Not m_update_flag Then
                    ...
                    If g_england_flag Then
                        ...
                        Else
                            ...
                            ' ----------------------------------------------
                            p_sql &= "Update run_info_header " & _
                                     "Set unload_user_id = '" & g_pass_str2 & _
                                     "', disinfect_lot = " & Me.return_parsed_single_quote_string(lbl_disinfect_lot.Text) & _
                                     ", process_num = '" & lbl_process_num.Text & _
                                     "' Where run_header_id = '" & m_run_header_id & "'; "
                    ...
                    Else
                        ...
                        ' ------------------
                        p_sql &= "Update run_info_header " & _
                                 "Set process_num = '" & lbl_process_num.Text & _
                                 "' Where run_header_id = '" & m_run_header_id & "'; "
                    End If
                End If
                ...
                If Not g_override_patient_entry_flag Then
                    ...
                    p_sql &= "INSERT INTO run_info_data (run_header_id, patient_id, instrument_id) " & _
                             "Select '" & m_run_header_id & "', patient_id, instrument_id " & _
                ...
                If m_prev_function = "MAIN" And g_override_patient_entry_flag = False Then
                    ...
                    For Each p_db_row In p_dataset.Tables(0).Rows
                        ...
                        ' -------------------------------------------
                        p_sql = "Select pre_run_id FROM pre_run_info " & _
                                "Where bay_number <> " & m_machine_index + 1 & _
                        ...
                        If p_dataset2.Tables(0).Rows.Count = 0 Then
                            ...
                            p_sql &= "DELETE FROM patient_maint " & _
                                     "WHERE patient_id = '" & p_db_row.Item(0).ToString & "'; "
                ...
                If Not g_override_patient_entry_flag Then
                    ...
                    ' --------------------------------
                    p_sql = "Select Distinct run_data_id " & _
                    ...
                    For Each p_db_row In p_dataset.Tables(0).Rows
                        p_sql = "Update run_info_data " & _
                                "Set procedure_num = " & get_procedure_num() & _
...
#Region " Set Screen "
    Public Sub set_screen(ByVal p_pinout As CUSNET.PCBStructs.SYSTEM_ID_DEF, _
        ...
        Select Case p_pinout
            ...
            Case CUSNET.PCBStructs.SYSTEM_ID_DEF.S82, _
                 CUSNET.PCBStructs.SYSTEM_ID_DEF.S83, _
...
#Region " Set Cycle Buttons "
    Private Sub Set_Cycle_Buttons(ByVal p_profile_id As String)
        ...
        ' * Set Cycle Buttons
        ' -------------------
        p_sql = "Select button_text, profile_id From profile_maint " & _
                "Where profile_id = '" & p_profile_id & _
                "'; Select button_text, profile_id " & _
                "From profile_maint P1 " & _
                "INNER JOIN profile_subs S1 On S1.sub_profile_id = P1.profile_id " & _
                "Where main_profile_id = '" & p_profile_id & _
                "' Order by order_num;"
        p_dataset = g_framework.Get_Data(p_sql)
...
#Region " Set Cycle Overrides "
    Private Sub set_cycle_overrides(ByVal p_type As String, ByRef p_cycle As CUSNET.Cycle, _
